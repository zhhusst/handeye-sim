...
namespace Lmi3d
{
    namespace GoSdk
    {
        ...
        public ref class GoGeoCal : public KObject
        {
            ...
            /// <summary>Loads the GoGeoCal data from file.</summary>
            /// <param name="fileName">Name of the file.</param>
            static GoGeoCal^ Load(String^ fileName)
            {
                ::GoGeoCal geoCal = kNULL;
                KString str(fileName);

                KCheck(::GoGeoCal_Load(&geoCal, str.CharPtr, kNULL));

                return KToObject<GoGeoCal^>(geoCal);
            }

            /// <summary>Saves the GoGeoCal object to file.</summary>
            /// <param name="fileName">Name of the file.</param>
            void Save(String^ fileName)
            {
                KString str(fileName);
                KCheck(::GoGeoCal_Save(Handle, str.CharPtr));
            }
