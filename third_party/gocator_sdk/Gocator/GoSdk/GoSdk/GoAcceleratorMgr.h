 ...
 *  
 *    // Accelerator logging is disabled by default.
 *    // Enable default GoXProcess logger.
 *    // If a sensor is currently accelerated, this will have no effect for that sensor.
 *    GoAcceleratorMgr_EnableLogging(kTRUE);
 * 
...
typedef enum GoAcceleratorMgrAccelEvents
{
    ...
    GO_ACCELERATOR_MGR_EVENT_TIME_DATE,
...
* Default logger will write data to file.
* Log filename: sensorId_GoXProcess.log
*
* @public                 @memberof GoAcceleratorMgr
* @version                Introduced in firmware x.x.x.x
* @param   manager        GoAcceleratorMgr object.
* @return                 True, if default logging is enabled, false otherwise.
*/
GoFx(kBool) GoAcceleratorMgr_IsLoggingEnabled(GoAcceleratorMgr manager);

/**
* Enables default Gocator accelerator logger.
* Log location: ./data/GoXProcess/
* Log filename: sensorId_GoXProcess.log
* Max size of single log file: 2 MB
* Max number of log files: 2
* Only 2 latest log files are kept. Older log files will be removed.
*
* @public                 @memberof GoAcceleratorMgr
* @version                Introduced in firmware x.x.x.x
* @param   manager        GoAcceleratorMgr object.
* @param   enabled        Whether logger should be enabled or disabled.
* @return                 Operation status.
*/
GoFx(kStatus) GoAcceleratorMgr_EnableLogging(GoAcceleratorMgr manager, kBool enabled);
