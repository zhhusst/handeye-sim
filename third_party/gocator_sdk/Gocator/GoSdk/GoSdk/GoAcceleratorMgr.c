...
GoFx(const kChar*) GoAcceleratorMgr_GetEventName(GoAcceleratorEventType event)
{
    ...
    case GO_ACCELERATOR_EVENT_PROCESS_STOPPED:
        name = "Stopped";
        break;
    case GO_ACCELERATOR_EVENT_SET_TIME_DATE:
        // GOC-14814 - Define a new time date set event.
        name = "Time Date Set";
        break;
    default:
        name = "Unknown Event";
...
GoFx(kStatus) GoAcceleratorMgr_SetupProcessArguments(GoAcceleratorMgr manager, GoAcceleratorMgrSensorEntry* sensorInfo)
{
    ...
    kCheck(kStrPrintf(processArg, kCountOf(processArg), "%u", obj->isLoggingEnabled));
    kCheck(kProcess_AddArgument(process, (kChar*)processArg));
...
GoFx(kStatus) GoAcceleratorMgr_WorkerStopAcceleration(GoAcceleratorMgr manager, GoAcceleratorMgrWorkerMsg* msg)
{
    ...
    kTry
    {
        ...
        if (!kIsNull(sensorInfo.sensorProcess))
        {
            ...
            status = kProcess_WaitImpl(sensorInfo.sensorProcess, GO_ACCELERATOR_MGR_TIMEOUT_GRACEFUL_TERMINATION);

            if (status == kERROR_TIMEOUT)
            {
                status = kProcess_Terminate(sensorInfo.sensorProcess);
            }
            kTest(kProcess_Reap(sensorInfo.sensorProcess));
            ...
            if (kSuccess(kLock_Enter(obj->updateCallback.lock)))
            {
                if (!kIsNull(obj->updateCallback.onAccelUpdate.function))
                {
                    ...
                    case GO_ACCELERATOR_EVENT_PROCESS_STOPPED:
                        ...
                        break;
                    case GO_ACCELERATOR_EVENT_SET_TIME_DATE:
                        updateStatus.accelEvent = GO_ACCELERATOR_MGR_EVENT_TIME_DATE;
                        break;
                    default:
                        // Unexpected event.
                        doUpdate = kFALSE;
