...
GoFx(kStatus) GoSetupNode_Init(GoSetupNode node, kType type, kObject setup, GoRole role, kObject sensor, kAlloc alloc)
{
    ...
    obj->preFilter.used = kFALSE;
    obj->preFilter.enabled = kFALSE;
    obj->preFilter.type = GO_PREFILTER_TYPE_GAUSSIAN;
    obj->preFilter.length = GO_PREFILTER_LENGTH_OPTION_5;
    kZero(obj->preFilter.coefficients);
    obj->preFilter.coefficientValueMin = 0.0;
    obj->preFilter.coefficientValueMax = 0.0;
    obj->preFilter.coefficientsExpectedSum = 0.0;
    obj->preFilter.typeOptions = kNULL;
    obj->preFilter.lengthOptions = kNULL;
    obj->preFilter.coefficientsCustomUserValues = kNULL;
    obj->preFilter.preFilterUsedSet = kFALSE;
    kZero(obj->exposure);
    ...
    kZero(obj->xSubsampling);
    kZero(obj->xSubsamplingDefaultEnabled);
    kZero(obj->zSubsamplingOptions);
    obj->zSubsamplingOptionCount = 0;
    kZero(obj->zSubsampling);
    kZero(obj->zSubsamplingDefaultEnabled);
    kZero(obj->tracking);
    obj->advanced = kNULL;
    obj->tracheid = kNULL;
    ...
    kTry
    {
        ...
        kTest(kArrayList_Construct(&obj->preFilter.typeOptions, kTypeOf(GoPreFilterType), 0, alloc));
        kTest(kArrayList_Construct(&obj->preFilter.lengthOptions, kTypeOf(GoPreFilterLengthOption), 0, alloc));
        kTest(kArrayList_Construct(&obj->preFilter.coefficientsCustomUserValues, kTypeOf(k64f), 0, alloc));
    }
    kCatch(&exception)
    {
...
GoFx(kStatus) GoSetupNode_VRelease(GoSetupNode node)
{
    ...
    kCheck(kDestroyRef(&obj->preFilter.coefficientsCustomUserValues));
    kCheck(kDestroyRef(&obj->preFilter.lengthOptions));
    kCheck(kDestroyRef(&obj->preFilter.typeOptions));
...
GoFx(kStatus) GoSetupNode_ReadPreFilter(GoSetupNode node, kXml xml, kXmlItem item)
{
    kObj(GoSetupNode, node);

    kSize coeffCount;
    kXmlItem preFilterChildItem;
    kText256 tempText;

    kCheck(kXml_AttrBool(xml, item, "used", &obj->preFilter.used));

    // Prefilter enabled is mandatory.
    kCheckArgs(!kIsNull(preFilterChildItem = kXml_Child(xml, item, "Enabled")));
    kCheck(kXml_ItemBool(xml, preFilterChildItem, &obj->preFilter.enabled));

    // Prefilter type is mandatory.
    kCheckArgs(!kIsNull(preFilterChildItem = kXml_Child(xml, item, "Type")));
    kCheck(kXml_Item32s(xml, preFilterChildItem, &obj->preFilter.type));
    kCheck(kXml_AttrText(xml, preFilterChildItem, "options", tempText, kCountOf(tempText)));
    kCheck(kArrayList_Clear(obj->preFilter.typeOptions));
    kCheck(GoOptionList_ParseList32s(tempText, obj->preFilter.typeOptions));

    // Prefilter length is mandatory.
    kCheckArgs(!kIsNull(preFilterChildItem = kXml_Child(xml, item, "Length")));
    kCheck(kXml_Item32s(xml, preFilterChildItem, &obj->preFilter.length));
    kCheck(kXml_AttrText(xml, preFilterChildItem, "options", tempText, kCountOf(tempText)));
    kCheck(kArrayList_Clear(obj->preFilter.lengthOptions));
    kCheck(GoOptionList_ParseList32s(tempText, obj->preFilter.lengthOptions));

    // Prefilter coefficients is mandatory.
    kCheckArgs(!kIsNull(preFilterChildItem = kXml_Child(xml, item, "Coefficients")));
    kCheck(kXml_Attr64f(xml, preFilterChildItem, "min", &obj->preFilter.coefficientValueMin));
    kCheck(kXml_Attr64f(xml, preFilterChildItem, "max", &obj->preFilter.coefficientValueMax));
    kCheck(kXml_Attr64f(xml, preFilterChildItem, "sum", &obj->preFilter.coefficientsExpectedSum));
    kCheck(kXml_AttrText(xml, preFilterChildItem, "userValue", tempText, kCountOf(tempText)));
    kCheck(kArrayList_Clear(obj->preFilter.coefficientsCustomUserValues));
    kCheck(GoOptionList_ParseList64f(tempText, obj->preFilter.coefficientsCustomUserValues));
    kCheck(kXml_ItemText(xml, preFilterChildItem, tempText, kCountOf(tempText)));
    // The "coeffCount" should in theory be the same as obj->preFilterNumCoefficientOption.
    // If the 2 numbers are different, then there is something amiss.
    // However, since the prefilter coefficients are usually determined and set by the SDK application,
    // the coefficients read from the device has little practical use to the SDK application.
    // It is only useful as a check of what coefficients the device is actually using.
    //
    // If SDK application wants to access obj->preFilterCoefficients, it should assume the array
    // has obj->preFilterNumCoefficientOption entries.
    // Since the obj->preFilterCoefficients array is a fixed size array large enough for the longest/largest
    // filter, as long as obj->preFilterNumCoefficientOption is valid, access to the
    // array will not exceed the bounds of the coefficients array.
    //
    // TODO: warn user somehow there is a mismatch.
    //   Remember to handle case where:
    //     - prefiltering is not supported so # of elements in the coefficients array is not meaningful.
    //     - prefiltering is disabled so # of elements in the array and # of configured coefficients are not meaningful.
    kCheck(GoOptionList_Parse64f(tempText, obj->preFilter.coefficients, kCountOf(obj->preFilter.coefficients), &coeffCount));
...
GoFx(kStatus) GoSetupNode_ReadConfig(GoSetupNode node, kXml xml, kXmlItem item)
{
    ...
    if (!kIsNull(tempItem))
    {
        ...
        kCheck(kXml_Item32u(xml, tempItem, &obj->xSubsampling.value));

        if (kSuccess(kXml_Attr32u(xml, tempItem, "value", &temp32u)))
        {
            obj->xSubsampling.systemValue = temp32u;
        }
        else
        {
            obj->xSubsampling.systemValue = obj->xSubsampling.value;
        }
        if (kSuccess(kXml_AttrBool(xml, tempItem, "used", &tempBool)))
        {
            obj->xSubsampling.enabled = tempBool;
        }
        else
        {
            obj->xSubsampling.enabled = kTRUE;
        }
    }

    if (kXml_ChildExists(xml, item, "XSubsamplingDefaultEnabled"))
    {
        kCheckArgs(!kIsNull(tempItem = kXml_Child(xml, item, "XSubsamplingDefaultEnabled")));
        kCheck(kXml_ItemBool(xml, tempItem, &obj->xSubsamplingDefaultEnabled.value));
        kCheck(kXml_AttrBool(xml, tempItem, "used", &obj->xSubsamplingDefaultEnabled.enabled));
    }
    ...
    if (!kIsNull(tempItem))
    {
        ...
        kCheck(kXml_Item32u(xml, tempItem, &obj->zSubsampling.value));

        if (kSuccess(kXml_Attr32u(xml, tempItem, "value", &temp32u)))
        {
            obj->zSubsampling.systemValue = temp32u;
        }
        else
        {
            obj->zSubsampling.systemValue = obj->zSubsampling.value;
        }
        if (kSuccess(kXml_AttrBool(xml, tempItem, "used", &tempBool)))
        {
            obj->zSubsampling.enabled = tempBool;
        }
        else
        {
            obj->zSubsampling.enabled = kTRUE;
        }

    }

    if (kXml_ChildExists(xml, item, "ZSubsamplingDefaultEnabled"))
    {
        kCheckArgs(!kIsNull(tempItem = kXml_Child(xml, item, "ZSubsamplingDefaultEnabled")));
        kCheck(kXml_ItemBool(xml, tempItem, &obj->zSubsamplingDefaultEnabled.value));
        kCheck(kXml_AttrBool(xml, tempItem, "used", &obj->zSubsamplingDefaultEnabled.enabled));
    }
    ...
    // Prefilter section. Optional for backwards/forwards compatibility.
    tempItem = kXml_Child(xml, item, "PreFilter");
    if (!kIsNull(tempItem))
    {
        kCheck(GoSetupNode_ReadPreFilter(node, xml, tempItem));
    }

    // GOC-14988: Extract fields associated with G3 Comptibility Mode
    if (kXml_ChildExists(xml, item, "IntensityCompatibilityModeEnabled"))
    {
        tempItem = kXml_Child(xml, item, "IntensityCompatibilityModeEnabled");
        kCheck(GoAdvanced_ReadIntensityCompatibility(obj->advanced, xml, tempItem));
    }
...
GoFx(kStatus) GoSetupNode_WriteConfig(GoSetupNode node, kXml xml, kXmlItem item)
{
    ...
    kCheck(kXml_SetChild32u(xml, item, "XSubsampling", obj->xSubsampling.value));
    kCheck(kXml_SetChildBool(xml, item, "XSubsamplingDefaultEnabled", obj->xSubsamplingDefaultEnabled.value));
    kCheck(kXml_SetChild32u(xml, item, "ZSubsampling", obj->zSubsampling.value));
    kCheck(kXml_SetChildBool(xml, item, "ZSubsamplingDefaultEnabled", obj->zSubsamplingDefaultEnabled.value));
    ...
    kCheck(kXml_AddItem(xml, item, "PreFilter", &tempItem));
    kCheck(kXml_SetChildBool(xml, tempItem, "Enabled", obj->preFilter.enabled));
    kCheck(kXml_SetChild32s(xml, tempItem, "Type", obj->preFilter.type));
    kCheck(kXml_SetChild32s(xml, tempItem, "Length", obj->preFilter.length));
    kCheck(GoOptionList_Format64f(obj->preFilter.coefficients, obj->preFilter.length, tempText, kCountOf(tempText)));
    kCheck(kXml_SetChildText(xml, tempItem, "Coefficients", tempText));

    // GOC-14988: Set field associated with G3 compatibility Mode.
    kCheck(kXml_SetChildBool(xml, item, "IntensityCompatibilityModeEnabled", GoAdvanced_IntensityCompatibilityMode(obj->advanced)));

    // Handle forward compatibility of the various configuration file settings.
...
GoFx(kStatus) GoSetup_Init(GoSetup setup, kType type, kObject sensor, kAlloc alloc)
{
    ...
    obj->transmitLimit = GO_TRANSMIT_RATE_PERCENT_MAX;
    obj->transmitLimitMin = GO_TRANSMIT_RATE_PERCENT_MIN;
    obj->transmitLimitMax = GO_TRANSMIT_RATE_PERCENT_MAX;
...
GoFx(kStatus) GoSetup_ReadConfig(GoSetup setup, kXml xml, kXmlItem item)
{
    ...
    // GOC-15511: Added transmit limit to the job file so SDK users can configure it.
    if (kXml_ChildExists(xml, item, "TransmitLimit"))
    {
        tempItem = kXml_Child(xml, item, "TransmitLimit");
        kCheck(kXml_Item32u(xml, tempItem, &obj->transmitLimit));
        kCheck(kXml_Attr32u(xml, tempItem, "min", &obj->transmitLimitMin));
        kCheck(kXml_Attr32u(xml, tempItem, "max", &obj->transmitLimitMax));
    }
...
GoFx(kStatus) GoSetup_WriteConfig(GoSetup setup, kXml xml, kXmlItem item)
{
    ...
    kCheck(kXml_SetChild32u(xml, item, "TransmitLimit", obj->transmitLimit));
...
GoFx(kBool) GoSetup_IsEnhancedSurfaceFlatnessFilterSupported(GoSetup setup, GoRole role)
{
    kObj(GoSetupNode, GoSetup_FindNode(setup, role));
    kObjN(GoSetup, setupObj, obj->setup);

    if (obj->preFilter.preFilterUsedSet == kFALSE)
    {
        GoSensor_SyncConfig(setupObj->sensor);
        obj->preFilter.preFilterUsedSet = kTRUE;
    }

    return obj->preFilter.used;
}

GoFx(kStatus) GoSetup_EnableEnhancedSurfaceFlatnessFilter(GoSetup setup, GoRole role, kBool enable)
{
    kObj(GoSetupNode, GoSetup_FindNode(setup, role));
    kObjN(GoSetup, setupObj, obj->setup);

    if (!GoSetup_IsEnhancedSurfaceFlatnessFilterSupported(setup, role))
    {
        return kERROR_UNIMPLEMENTED;
    }

    obj->preFilter.enabled = enable;
...
// Built-in prefilters (eg. Box, Gaussian filters) has predefined sets of coefficients, one set
// for each supported filter length. Users therefore only have to select which built-in prefilter
// to use and the length of the prefilter.
// The actual coefficients are not changeable by the user.
GoFx(kStatus) GoSetup_SelectBuiltInEnhancedSurfaceFlatnessFilter(GoSetup setup, GoRole role, GoPreFilterType type, GoPreFilterLengthOption length)
{
    kObj(GoSetupNode, GoSetup_FindNode(setup, role));
    kObjN(GoSetup, setupObj, obj->setup);
    ...
    // Don't use ths API to enable custom prefiltering. Use GoSetup_SelectCustomEnhancedSurfaceFlatnessFilter() instead.
    kCheckArgs(type != GO_PREFILTER_TYPE_CUSTOM);

    kCheckArgs(GoOptionList_Check32s(kArrayList_DataT(obj->preFilter.typeOptions, const k32s),
        kArrayList_Count(obj->preFilter.typeOptions),
        type));

    kCheckArgs(GoOptionList_Check32s(kArrayList_DataT(obj->preFilter.lengthOptions, const k32s),
        kArrayList_Count(obj->preFilter.lengthOptions),
        length));

    obj->preFilter.type = type;
    obj->preFilter.length = length;
...
// This function checks that each coefficient value is within the min/max range,
// and that the sum is equal the limit (subject to limitations of representing
...
GoFx(kBool) GoSetup_IsEnhancedSurfaceFlatnessFilterCoefficientsValid(GoSetup setup, GoRole role, const GoPreFilterCoefficient* coefficientsArray, k32u numCoefficients)
{
    ...
    kBool isValid = kTRUE;
    GoPreFilterCoefficient sum = 0.0;

    for (k32u i = 0; i < numCoefficients; i++)
    {
        GoPreFilterCoefficient val = coefficientsArray[i];

        if ((val < GoSetup_EnhancedSurfaceFlatnessFilterCoefficientMinValue(setup, role)) || (val > GoSetup_EnhancedSurfaceFlatnessFilterCoefficientMaxValue(setup, role)))
        {
            isValid = kFALSE;
            break;
        }
    ...
    return (isValid && (kMath_NearlyEquals64f(sum, GoSetup_EnhancedSurfaceFlatnessFilterCoefficientsExpectedSum(setup, role))));
}

// This helper function is to select the CUSTOM prefilter type, if it is not already selected.
// If it is already selected, then no need to do it again, to avoid unnecessary network communications
// with the device.
GoFx(kStatus) GoSetup_SelectCustomEnhancedSurfaceFlatnessFilterMode(GoSetup setup, GoRole role)
{
    ...
    // Check the local preFilterMode to see if the SDK has already set the device to CUSTOM mode.
    // This can avoid having to fetch the configuration setting over the network from the device unnecessarily.
    // If not currently in CUSTOM mode, the device could potentially already be in CUSTOM mode (eg default job
    // file puts device into CUSTOM prefilter mode, but the SDK has not read the device's
    // configuration value yet), so do a final check to read the device's current preFilterMode.
    if ((obj->preFilter.type != GO_PREFILTER_TYPE_CUSTOM) && (GoSetup_EnhancedSurfaceFlatnessFilterType(setup, role) != GO_PREFILTER_TYPE_CUSTOM))
    {
        // Device really is not in CUSTOM mode.
        obj->preFilter.type = GO_PREFILTER_TYPE_CUSTOM;
        kCheck(GoSensor_SetConfigModified(setupObj->sensor));

        // Make the change take effect immediately on the device by flushing the config.
        // This flushing is automatically done on a read of a configuration parameter,
        // such as reading the prefilter mode.
        kCheckTrue(GoSetup_EnhancedSurfaceFlatnessFilterType(setup, role) == GO_PREFILTER_TYPE_CUSTOM, kERROR_STATE);
    }

    return kOK;
}

// Custom prefilter uses prefilter coefficients defined by the user.
// User emters the coefficient values and the length of the filter to use.
// For convenience to the user, since there is only one custom mode, the user
// does not have to explicitly select the custom prefilter - this will be done
// automatically by this function call.
GoFx(kStatus) GoSetup_SelectCustomEnhancedSurfaceFlatnessFilter(GoSetup setup, GoRole role, GoPreFilterLengthOption length, const GoPreFilterCoefficient* coefficientsArray)
{
    kObj(GoSetupNode, GoSetup_FindNode(setup, role));
    kObjN(GoSetup, setupObj, obj->setup);
    ...
    kCheckArgs(coefficientsArray != kNULL);

    kCheckTrue(GoSetup_IsEnhancedSurfaceFlatnessFilterCoefficientsValid(setup, role, coefficientsArray, length), kERROR_PARAMETER);

    // This function selects the CUSTOM prefilter mode on the device, if the device is not already in CUSTOM mode.
    // IMPORTANT:
    //   - For UI users, configurating a custom prefilter requires separate manual steps just because of the nature
    //     of the UI interface. The individual steps a UI user goes through using the UI are:
    //      1. select CUSTOM  mode
    //      2. change the number of coefficients to use
    //      3. change the filter coefficients
    //   - UI users cannot enter into CUSTOM mode from a non-custom mode AND change the
    //         number of coefficients and/or filter coefficients at the same time.
    //   - For UI users' convenience, when UI user first selects CUSTOM mode (from a non-custom mode)
    //     in step 1, the previous custom settings are restored for the user.
    //   - User performs steps 2 and 3 while already in CUSTOM mode.
    //   - The above UI user behaviour has to be followed by a programmatic interface. The programmatic
    //     interface should not try to change the number of coefficients and/or filter coefficients until
    //     the device is already in CUSTOM mode.
    kCheck(GoSetup_SelectCustomEnhancedSurfaceFlatnessFilterMode(setup, role));

    // Once in CUSTOM mode, the number of coefficients and/or coefficient parameters changes will be accepted by the
    // device.
    obj->preFilter.length = length;

    kCheck(kMemCopy(obj->preFilter.coefficients, coefficientsArray, length * sizeof(GoPreFilterCoefficient)));
...
GoFx(kBool) GoSetup_EnhancedSurfaceFlatnessFilterEnabled(GoSetup setup, GoRole role)
{
    kObj(GoSetupNode, GoSetup_FindNode(setup, role));
    kObjN(GoSetup, setupObj, obj->setup);
    ...
    return obj->preFilter.enabled;
}

GoFx(GoPreFilterType) GoSetup_EnhancedSurfaceFlatnessFilterType(GoSetup setup, GoRole role)
{
    kObj(GoSetupNode, GoSetup_FindNode(setup, role));
    kObjN(GoSetup, setupObj, obj->setup);
    ...
    return obj->preFilter.type;
}

GoFx(GoPreFilterLengthOption) GoSetup_EnhancedSurfaceFlatnessFilterLength(GoSetup setup, GoRole role)
{
    kObj(GoSetupNode, GoSetup_FindNode(setup, role));
    kObjN(GoSetup, setupObj, obj->setup);
    ...
    return obj->preFilter.length;
}

GoFx(const GoPreFilterCoefficient*) GoSetup_EnhancedSurfaceFlatnessFilterCoefficients(GoSetup setup, GoRole role)
{
    kObj(GoSetupNode, GoSetup_FindNode(setup, role));
    kObjN(GoSetup, setupObj, obj->setup);
    ...
    return obj->preFilter.coefficients;
}

GoFx(k64f) GoSetup_EnhancedSurfaceFlatnessFilterCoefficientMinValue(GoSetup setup, GoRole role)
{
    kObj(GoSetupNode, GoSetup_FindNode(setup, role));

    // This value never changes so no need to resync the configuration data with the device.
    return obj->preFilter.coefficientValueMin;
}

GoFx(k64f) GoSetup_EnhancedSurfaceFlatnessFilterCoefficientMaxValue(GoSetup setup, GoRole role)
{
    kObj(GoSetupNode, GoSetup_FindNode(setup, role));

    // This value never changes so no need to resync the configuration data with the device.
    return obj->preFilter.coefficientValueMax;
}

GoFx(k64f) GoSetup_EnhancedSurfaceFlatnessFilterCoefficientsExpectedSum(GoSetup setup, GoRole role)
{
    kObj(GoSetupNode, GoSetup_FindNode(setup, role));

    // This value never changes so no need to resync the configuration data with the device.
    return obj->preFilter.coefficientsExpectedSum;
}
...
GoFx(kStatus) GoSetup_SetXSubsampling( GoSetup setup, GoRole role, k32u xSubsampling )
{
    ...
    obj->xSubsampling.value = xSubsampling;
...
GoFx(k32u) GoSetup_XSubsampling(GoSetup setup, GoRole role)
{
    ...
    return obj->xSubsampling.value;
}

GoFx(k32u) GoSetup_XSubsamplingSystemValue(GoSetup setup, GoRole role)
{
    kObj(GoSetupNode, GoSetup_FindNode(setup, role));
    kObjN(GoSetup, setupObj, obj->setup);
    ...
    return obj->xSubsampling.systemValue;
}

GoFx(kBool) GoSetup_XSubsamplingUsed(GoSetup setup, GoRole role)
{
    kObj(GoSetupNode, GoSetup_FindNode(setup, role));
    kObjN(GoSetup, setupObj, obj->setup);
    ...
    return obj->xSubsampling.enabled;
}

GoFx(kStatus) GoSetup_SetXSubsamplingDefaultEnabled(GoSetup setup, GoRole role, kBool useDefault)
{
    kObj(GoSetupNode, GoSetup_FindNode(setup, role));
    kObjN(GoSetup, setupObj, obj->setup);
    GoSensor_SyncConfig(setupObj->sensor);

    obj->xSubsamplingDefaultEnabled.value = useDefault;
    kCheck(GoSensor_SetConfigModified(setupObj->sensor));

    return kOK;
}

GoFx(kBool) GoSetup_XSubsamplingDefaultEnabled(GoSetup setup, GoRole role)
{
    kObj(GoSetupNode, GoSetup_FindNode(setup, role));
    kObjN(GoSetup, setupObj, obj->setup);
    GoSensor_SyncConfig(setupObj->sensor);

    return obj->xSubsamplingDefaultEnabled.value;
}

GoFx(kBool) GoSetup_XSubsamplingDefaultEnabledUsed(GoSetup setup, GoRole role)
{
    kObj(GoSetupNode, GoSetup_FindNode(setup, role));
    kObjN(GoSetup, setupObj, obj->setup);
    ...
    return obj->xSubsamplingDefaultEnabled.enabled;
}
...
GoFx(kStatus) GoSetup_SetZSubsampling(GoSetup setup, GoRole role, k32u zSubsampling)
{
    ...
    obj->zSubsampling.value = zSubsampling;
...
GoFx(k32u) GoSetup_ZSubsampling(GoSetup setup, GoRole role)
{
    ...
    return obj->zSubsampling.value;
}

GoFx(k32u) GoSetup_ZSubsamplingSystemValue(GoSetup setup, GoRole role)
{
    kObj(GoSetupNode, GoSetup_FindNode(setup, role));
    kObjN(GoSetup, setupObj, obj->setup);
    ...
    return obj->zSubsampling.systemValue;
}

GoFx(kBool) GoSetup_ZSubsamplingUsed(GoSetup setup, GoRole role)
{
    kObj(GoSetupNode, GoSetup_FindNode(setup, role));
    kObjN(GoSetup, setupObj, obj->setup);
    ...
    return obj->zSubsampling.enabled;
}

GoFx(kStatus) GoSetup_SetZSubsamplingDefaultEnabled(GoSetup setup, GoRole role, kBool useDefault)
{
    kObj(GoSetupNode, GoSetup_FindNode(setup, role));
    kObjN(GoSetup, setupObj, obj->setup);
    GoSensor_SyncConfig(setupObj->sensor);

    obj->zSubsamplingDefaultEnabled.value = useDefault;
    kCheck(GoSensor_SetConfigModified(setupObj->sensor));

    return kOK;
}

GoFx(kBool) GoSetup_ZSubsamplingDefaultEnabled(GoSetup setup, GoRole role)
{
    kObj(GoSetupNode, GoSetup_FindNode(setup, role));
    kObjN(GoSetup, setupObj, obj->setup);
    GoSensor_SyncConfig(setupObj->sensor);

    return obj->zSubsamplingDefaultEnabled.value;
}

GoFx(kBool) GoSetup_ZSubsamplingDefaultEnabledUsed(GoSetup setup, GoRole role)
{
    kObj(GoSetupNode, GoSetup_FindNode(setup, role));
    kObjN(GoSetup, setupObj, obj->setup);
    ...
    return obj->zSubsamplingDefaultEnabled.enabled;
}
...
GoFx(kStatus) GoSetup_SetTransmitLimit(GoSetup setup, k32u limit)
{
    kObj(GoSetup, setup);

    // Ensure the new transmitLimit falls in the valid range.
    if (limit > obj->transmitLimitMax || limit < obj->transmitLimitMin) return kERROR_PARAMETER;

    kCheckState(GoSensor_IsConfigurable(obj->sensor));
    kCheck(GoSensor_CacheConfig(obj->sensor));

    obj->transmitLimit = limit;
    kCheck(GoSensor_SetConfigModified(obj->sensor));

    return kOK;
}

GoFx(k32u) GoSetup_TransmitLimit(GoSetup setup)
{
    kObj(GoSetup, setup);

    GoSensor_SyncConfig(obj->sensor);

    return obj->transmitLimit;
}
