...
*
* @internal
* Copyright (C) 2017-2024 by LMI Technologies Inc.
...
GoFx(kStatus) GoGeoCal_Load(GoGeoCal* cal, const kChar* fileName, kAlloc allocator)
{
    kObject xml = kNULL;
    ...
    kTry
    {
        kTest(kXml_Load(&xml, fileName, allocator));
        kTest(kObject_Is(xml, kTypeOf(kXml)));
        kTest(GoGeoCal_Construct(&newGeoCal, xml, allocator));

        *cal = newGeoCal;
    }
    kCatchEx(&exception)
    {
        kCheck(kDisposeRef(&xml));
        kCheck(kDisposeRef(&newGeoCal));
...
GoFx(kStatus) GoGeoCal_Write(GoGeoCal cal, kXml xml, kXmlItem root)
{
    kObj(GoGeoCal, cal);
    kXmlItem attributesItem = kNULL;
    kXmlItem camerasItem = kNULL;
    kXmlItem cameraItem = kNULL;
    kXmlItem calWindowItem = kNULL;
    kXmlItem fitItem = kNULL;
    kText256 buffer;
    kSize i, j;

    if (root == kNULL)
    {
        kCheck(kXml_AddItem(xml, kNULL, "GeoCal", &root));
        kCheck(kXml_SetAttr32u(xml, root, "version", GO_GEO_CAL_VERSION));
    }

    kCheck(kXml_AddItem(xml, root, "Attributes", &attributesItem));
    kCheck(kXml_SetChild32u(xml, attributesItem, "Id", obj->id));
    kCheck(kXml_SetChildText(xml, attributesItem, "Timestamp", obj->timestamp));

    kCheck(kXml_AddItem(xml, root, "Cameras", &camerasItem));

    for (i = 0; i < kArray1_Length(obj->cameras); i++)
    {
        GoGeoCalCamera* camera = kArray1_AtT(obj->cameras, i, GoGeoCalCamera);

        kCheck(kXml_AddItem(xml, camerasItem, "Camera", &cameraItem));

        kCheck(kXml_AddItem(xml, cameraItem, "CalWindow", &calWindowItem));
        kCheck(kXml_SetChild32u(xml, calWindowItem, "X", camera->calWindow.x));
        kCheck(kXml_SetChild32u(xml, calWindowItem, "Y", camera->calWindow.y));
        kCheck(kXml_SetChild32u(xml, calWindowItem, "Width", camera->calWindow.width));
        kCheck(kXml_SetChild32u(xml, calWindowItem, "Height", camera->calWindow.height));
        kCheck(kXml_SetChild32u(xml, calWindowItem, "XSubsampling", camera->calWindow.xSubsampling));
        kCheck(kXml_SetChild32u(xml, calWindowItem, "YSubsampling", camera->calWindow.ySubsampling));

        kCheck(kXml_AddItem(xml, cameraItem, "XCenterAsZ", &fitItem));
        for (j = 0; j < kArray1_Length(camera->xCenterCoeffs); j++)
        {
            kCheck(kStrPrintf(buffer, kCountOf(buffer), "C%d", j));
            kCheck(kXml_SetChild64f(xml, fitItem, buffer, *(k64f*)kArray1_At(camera->xCenterCoeffs, j)));
        }

        kCheck(kXml_AddItem(xml, cameraItem, "YCenterAsZ", &fitItem));
        for (j = 0; j < kArray1_Length(camera->yCenterCoeffs); j++)
        {
            kCheck(kStrPrintf(buffer, kCountOf(buffer), "C%d", j));
            kCheck(kXml_SetChild64f(xml, fitItem, buffer, *(k64f*)kArray1_At(camera->yCenterCoeffs, j)));
        }

        if (!kIsNull(camera->xResCoeffs)) /* Optional */
        {
            kCheck(kXml_AddItem(xml, cameraItem, "XResolutionAsZ", &fitItem));
            for (j = 0; j < kArray1_Length(camera->xResCoeffs); j++)
            {
                kCheck(kStrPrintf(buffer, kCountOf(buffer), "C%d", j));
                kCheck(kXml_SetChild64f(xml, fitItem, buffer, *(k64f*)kArray1_At(camera->xResCoeffs, j)));
            }
        }

        if (!kIsNull(camera->yResCoeffs)) /* Optional */
        {
            kCheck(kXml_AddItem(xml, cameraItem, "YResolutionAsZ", &fitItem));
            for (j = 0; j < kArray1_Length(camera->yResCoeffs); j++)
            {
                kCheck(kStrPrintf(buffer, kCountOf(buffer), "C%d", j));
                kCheck(kXml_SetChild64f(xml, fitItem, buffer, *(k64f*)kArray1_At(camera->yResCoeffs, j)));
            }
        }

        kCheck(kXml_SetChild64f(xml, cameraItem, "Roll", camera->roll));
    }

    return kOK;
}

GoFx(kStatus) GoGeoCal_Save(GoGeoCal cal, const kChar* filename)
{
    kObject xml = kNULL;

    kTry
    {
        kTest(kXml_Construct(&xml, kObject_Alloc(cal)));
        kTest(GoGeoCal_Write(cal, xml, kXml_Root(xml)));
        kTest(kXml_Save(xml, filename));
    }
    kFinally
    {
