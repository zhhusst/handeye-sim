...
GoFx(kStatus) GoSensor_Shutdown(GoSensor sensor)
{
    kObj(GoSensor, sensor);
    ...
    kCheck(GoSensor_Flush(sensor));
    // Shutdown is functionally very similar to reset, so GoSensor_BeginReset can be reused.
    kCheck(GoSensor_BeginReset(sensor));
    kCheck(GoControl_Shutdown(obj->control));
    kCheck(GoSensor_Disconnect(sensor));
...
GoFx(GoSensorInfo) GoSensor_BuddiesAt(GoSensor sensor, k32u index)
{
    kObj(GoSensor, sensor);
    k32u buddyId;
    kSize buddySensorCount = kArrayList_Count(obj->buddySensorInfo);
    kSize remoteSensorCount = kArrayList_Count(obj->remoteSensorInfo);

    //GOC-14647 index represents which buddy is being returned.
    if (index >= buddySensorCount)
    {
        return kNULL;
    }

    buddyId = kArrayList_AtT(obj->buddySensorInfo, index, GoBuddyInfo)->id;

    //GOC-14647 Parse the remote sensors for the matching buddy id and return the corresponding remote sensor info.
    for (kSize i=0; i < remoteSensorCount; i++)
    {
         GoSensorInfo remoteSensor = *kArrayList_AtT(obj->remoteSensorInfo, i, GoSensorInfo);

         if (GoSensorInfo_Id(remoteSensor) == buddyId)
         {
             return remoteSensor;
         }
    }
...
GoFx(kStatus) GoSensor_SetTimeDate(GoSensor sensor)
{
    ...
    kCheck(GoControl_SetDateTime(obj->control, dateTime));

    return kOK;
}

// GOC-14715: Add laser shut off support to SDK, non-zero enable turns laser on, zeroed enable turns laser off.
GoFx(kStatus) GoSensor_SetLaserState(GoSensor sensor, k8u enable)
{
    kObj(GoSensor, sensor);

    kCheck(GoControl_SetLaserState(obj->control, enable));

    return kOK;
}

GoFx(kStatus) GoSensor_LockOutLaser(GoSensor sensor, kBool shouldLock)
{
    kObj(GoSensor, sensor);

    kCheck(GoControl_LockOutLaser(obj->control, shouldLock));
