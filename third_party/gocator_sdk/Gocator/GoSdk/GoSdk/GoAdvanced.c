...
GoFx(kStatus) GoAdvanced_Init(GoAdvanced advanced, kType type, kObject sensor, kAlloc alloc)
{
    ...
    obj->hdrMode = GO_HDR_MODE_DISABLED;
    obj->hdrModeUsed = kFALSE;
    obj->hdrModeUsedSet = kFALSE;
    obj->hdrModeOptions = kNULL;
    kZero(obj->hdrParameters);
    obj->hdrParametersCount = 0;
    obj->hdrParametersUsed = kFALSE;
    obj->hdrParametersValueMin = 0.0;
    obj->hdrParametersValueMax = 0.0;
    obj->hdrParametersCountMin = 0;
    obj->hdrParametersCountMax = 0;
    ...
    obj->g3CompatibilityMode.value = kFALSE;
    obj->g3CompatibilityMode.enabled = kFALSE;
    ...
    kTry
    {
        ...
        kTest(kArrayList_Construct(&obj->hdrModeOptions, kTypeOf(k32u), 0, alloc));
    }
    kCatch(&exception)
    {
...
GoFx(kStatus) GoAdvanced_VRelease(GoAdvanced advanced)
{
    ...
    kCheck(kDisposeRef(&obj->hdrModeOptions));
...
GoFx(kStatus) GoAdvanced_Read(GoAdvanced advanced, kXml xml, kXmlItem item)
{
    ...
    kCheck(GoAdvanced_ReadSpotSettings(advanced, xml, item));

    kCheck(GoAdvanced_ReadCameraSettings(advanced, xml, item));

    kCheck(GoAdvanced_ReadDynamicExposureSettings(advanced, xml, item));
...
GoFx(kStatus) GoAdvanced_ReadCameraSettings(GoAdvanced advanced, kXml xml, kXmlItem item)
{
    ...
    // HDR Mode  ------------------------------------------------------
    if (kXml_ChildExists(xml, item, "HDRMode"))
    {
        kCheckArgs(!kIsNull(tempItem = kXml_Child(xml, item, "HDRMode")));
        kCheck(kXml_Item32s(xml, tempItem, &obj->hdrMode));
        kCheck(kXml_AttrBool(xml, tempItem, "used", &obj->hdrModeUsed));

        kCheck(kXml_AttrText(xml, tempItem, "options", tempText, kCountOf(tempText)));
        kCheck(kArrayList_Clear(obj->hdrModeOptions));
        kCheck(GoOptionList_ParseList32u(tempText, obj->hdrModeOptions));
    }

    if (kXml_ChildExists(xml, item, "HDRParameters"))
    {
        kCheckArgs(!kIsNull(tempItem = kXml_Child(xml, item, "HDRParameters")));
        kCheck(kXml_ItemText(xml, tempItem, tempText, kCountOf(tempText)));
        kCheck(GoOptionList_Parse64f(tempText, obj->hdrParameters, kCountOf(obj->hdrParameters), &obj->hdrParametersCount));
        kCheck(kXml_AttrBool(xml, tempItem, "used", &obj->hdrParametersUsed));
        kCheck(kXml_Attr64f(xml, tempItem, "min", &obj->hdrParametersValueMin));
        kCheck(kXml_Attr64f(xml, tempItem, "max", &obj->hdrParametersValueMax));
        kCheck(kXml_Attr32u(xml, tempItem, "countMin", &obj->hdrParametersCountMin));
        kCheck(kXml_Attr32u(xml, tempItem, "countMax", &obj->hdrParametersCountMax));
    }
...
GoFx(kStatus) GoAdvanced_ReadIntensityCompatibility(GoAdvanced advanced, kXml xml, kXmlItem item)
{
    kObj(GoAdvanced, advanced);

    kCheck(kXml_ItemBool(xml, item, &obj->g3CompatibilityMode.value));
    kCheck(kXml_AttrBool(xml, item, "used", &obj->g3CompatibilityMode.enabled));
...
GoFx(kStatus) GoAdvanced_Write(GoAdvanced advanced, kXml xml, kXmlItem item)
{
    ...
    kCheck(GoAdvanced_WriteSpotSettings(advanced, xml, item));

    kCheck(GoAdvanced_WriteCameraSettings(advanced, xml, item));

    kCheck(GoAdvanced_WriteDynamicExposureSettings(advanced, xml, item));
...
GoFx(kStatus) GoAdvanced_WriteCameraSettings(GoAdvanced advanced, kXml xml, kXmlItem item)
{
    ...
    kCheck(kXml_SetChild32s(xml, item, "HDRMode", obj->hdrMode));
    kCheck(GoOptionList_Format64f(obj->hdrParameters, obj->hdrParametersCount, tempText, kCountOf(tempText)));
    kCheck(kXml_SetChildText(xml, item, "HDRParameters", tempText));
...
GoFx(GoHdrMode) GoAdvanced_HdrMode(GoAdvanced advanced)
{
    kObj(GoAdvanced, advanced);
...
GoFx(kStatus) GoAdvanced_SetHdrMode(GoAdvanced advanced, GoHdrMode mode)
{
    kObj(GoAdvanced, advanced);

    if (!GoAdvanced_IsHdrModeUsed(obj))
    {
        return kERROR_UNIMPLEMENTED;
    }

    kCheckArgs(GoOptionList_Check32u(kArrayList_DataT(obj->hdrModeOptions, const k32u),
        kArrayList_Count(obj->hdrModeOptions),
        mode));
...
GoFx(kBool) GoAdvanced_IsHdrModeUsed(GoAdvanced advanced)
{
    kObj(GoAdvanced, advanced);

    if (!obj->hdrModeUsedSet)
    {
        GoSensor_SyncConfig(obj->sensor);
        obj->hdrModeUsedSet = kTRUE;
    }

    return obj->hdrModeUsed;
}

GoFx(kSize) GoAdvanced_HdrModeOptionCount(GoAdvanced advanced)
{
    kObj(GoAdvanced, advanced);

    GoSensor_SyncConfig(obj->sensor);
    return kArrayList_Count(obj->hdrModeOptions);
}

GoFx(k64f) GoAdvanced_HdrParameters(GoAdvanced advanced, k32u index)
{
    kObj(GoAdvanced, advanced);

    GoSensor_SyncConfig(obj->sensor);

    if (index > (obj->hdrParametersCountMax - obj->hdrParametersCountMin))
    {
        return 0.0;
    }

    return obj->hdrParameters[index];
}

GoFx(kStatus) GoAdvanced_SetHdrParameters(GoAdvanced advanced, k32u index, k64f hdrParameter)
{
    kObj(GoAdvanced, advanced);
    
    if (!GoAdvanced_IsHdrModeUsed(advanced))
    {
        return kERROR_UNIMPLEMENTED;
    }
    ...
    if ((hdrParameter- DBL_EPSILON) < obj->hdrParametersValueMin || (hdrParameter+DBL_EPSILON) > obj->hdrParametersValueMax)
    {
        return kERROR_PARAMETER;
    }

    obj->hdrParameters[index] = kMath_Clamp_(hdrParameter, obj->hdrParametersValueMin, obj->hdrParametersValueMax);

    return kOK;
}

GoFx(kBool) GoAdvanced_IsHdrParametersUsed(GoAdvanced advanced)
{
    kObj(GoAdvanced, advanced);

    GoSensor_SyncConfig(obj->sensor);

    return obj->hdrParametersUsed;
}

GoFx(kSize) GoAdvanced_HdrParametersCount(GoAdvanced advanced)
{
    kObj(GoAdvanced, advanced);

    GoSensor_SyncConfig(obj->sensor);

    return obj->hdrParametersCount;
}

GoFx(kSize) GoAdvanced_HdrParametersCountMin(GoAdvanced advanced)
{
    kObj(GoAdvanced, advanced);

    GoSensor_SyncConfig(obj->sensor);

    return obj->hdrParametersCountMin;
}

GoFx(kSize) GoAdvanced_HdrParametersCountMax(GoAdvanced advanced)
{
    kObj(GoAdvanced, advanced);

    GoSensor_SyncConfig(obj->sensor);

    return obj->hdrParametersCountMax;
}

GoFx(k64f) GoAdvanced_HdrParametersValueMin(GoAdvanced advanced)
{
    kObj(GoAdvanced, advanced);

    GoSensor_SyncConfig(obj->sensor);

    return obj->hdrParametersValueMin;
}

GoFx(k64f) GoAdvanced_HdrParametersValueMax(GoAdvanced advanced)
{
    kObj(GoAdvanced, advanced);

    GoSensor_SyncConfig(obj->sensor);

    return obj->hdrParametersValueMax;
}
...
GoFx(kBool) GoAdvanced_IntensityCompatibilityMode(GoAdvanced advanced)
{
    kObj(GoAdvanced, advanced);

    GoSensor_SyncConfig(obj->sensor);

    return obj->g3CompatibilityMode.value;
}

GoFx(kBool) GoAdvanced_IntensityCompatibilityModeEnabled(GoAdvanced advanced)
{
    kObj(GoAdvanced, advanced);

    // NOTE: the GoAdvanced object is fetched using GoSetup_Advanced(),
    // which first does a read of the device's configuration.
    // For intensity compatibility, it never changes because it is hardware
    // dependent. So there is really no need to call GoSensor_SyncConfig()
    // here.
    // However, will keep teh GoSensor_SyncConfig() call here to maintain
    // the same coding pattern used by the other getter APIs.
    GoSensor_SyncConfig(obj->sensor);

    return obj->g3CompatibilityMode.enabled;
}

GoFx(kStatus) GoAdvanced_SetIntensityCompatibilityMode(GoAdvanced advanced, kBool state)
{
    kObj(GoAdvanced, advanced);

    // This flag's value never changes because compatibility is dependent on
    // the hardware and cannot be changed. So no need to refresh this flag
    // before using it.
    if (!obj->g3CompatibilityMode.enabled)
    {
        // Device does not need/have intensity compatibility feature.
        return kERROR_STATE;
    }

    obj->g3CompatibilityMode.value = state;
