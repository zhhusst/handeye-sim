...
kBeginEnumEx(k, kPixelFormat)
    ...
    kAddEnumerator(kPixelFormat, kPIXEL_FORMAT_8BPC_BGR)
...
kBeginValueEx(k, kRgb24)
    kAddPrivateVersionEx(kRgb24, "kdat6", "9.5.0.0", "kRgb24-0", Write, Read)

    kAddField(kRgb24, k8u, b)
    kAddField(kRgb24, k8u, g)
    kAddField(kRgb24, k8u, r)
kEndValueEx()

kFx(kBool) xkRgb24_VEquals(kType type, const void* value, const void* other)
{
    const kRgb24* a = (const kRgb24*)value;
    const kRgb24* b = (const kRgb24*)other;
...
{
    return kSerializer_Write8uArray(serializer, (const k8u*)values, 3 * count);
}

kFx(kStatus) xkRgb24_Read(kType type, void* values, kSize count, kSerializer serializer)
{
    return kSerializer_Read8uArray(serializer, (k8u*)values, 3 * count);
}
...
kBeginEnumEx(k, kThreadPriorityClass)
