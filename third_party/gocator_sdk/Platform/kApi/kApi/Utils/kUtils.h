 ...
 * pointers must be initialized to kNULL prior to the first call. If a token is found, 
 * this function will return kTRUE and the pointers referenced by the tokenStart and tokenEnd
 * parameters will be updated to indicate the token boundaries. 
 * 
 * If the discardEmpty parameter is kTRUE, this function will ignore any extra delimiters that are 
 * not strictly required to separate tokens; otherwise, a token will be emitted for every delimiter.
 * 
 * Unlike strtok, the original delimimiter will remain unmodified at the location referenced by tokenEnd 
 * upon return of this call. To provide strtok-like behavior, the caller can optionally set the 
 * character referenced by tokenEnd to the null terminator.
 * 
 * @public                  @memberof kUtils
 * @param   input           Input string to be searched. 
 * @param   delimiterList   List of delimiter characters.
 * @param   tokenStart      If token found, receives pointer to start of token; the refereneced pointer must be initialized to kNULL before first call.
 * @param   tokenEnd        If token found, receives pointer to one past end of token; the refereneced pointer must be initialized to kNULL before first call.
 * @param   context         Pointer to variable (maintained by caller) that stores parsing context between call invocations.
 * @param   discardEmpty    Should empty tokens be discarded?
 * @return                  kTRUE if a token was found. 
 */
#if defined (K_CPP)
kFx(kBool) kStrSplit(const kChar* input, const kChar* delimiterList, const kChar** tokenStart, const kChar** tokenEnd, kChar* context, kBool discardEmpty = kFALSE);
#endif
