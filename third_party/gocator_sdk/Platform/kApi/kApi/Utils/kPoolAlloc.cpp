 ...
 *
 * @internal
 * Copyright (C) 2013-2024 by LMI Technologies Inc.  All rights reserved.
 */
...
{
    auto* const userAllocation = kPointer_ByteOffset(buffer, xkPoolAllocBufferHeader_Space());
    ...
    {   
        if (!kIsNull(mem))
        {
            ...
            if (bufferHeaderCandidateAddress & xkPOOL_ALLOC_ALLOC_TYPE_MASK)
            {
                ...
                buffer = (xkPoolAllocBufferHeader*)kPointer_ByteOffset(mem, -1 * (kSSize)xkPoolAllocBufferHeader_Space());
            }
            else            
            {
            ...
            else if (buffer->type == obj->cacheAllocId)
            {
                ...
                {
                    obj->totalSize -= xkPoolAllocBufferHeader_Space() + rankInfo->size; 
            ...
            {
                obj->totalSize -= xkPoolAllocBufferHeader_Space() + buffer->info.size; 
...
kFx(kStatus) xkPoolAlloc_PrepareFreeBlockBasedBuffer(kPoolAlloc object, kSize rank)
{
    ...
    if (rankInfo->freeBufferList.count == 0)
    {
        ...
        if (rankInfo->newBlockList.count > 0)
        {
            ...
            xkPoolAllocBufferHeader* buffer = (xkPoolAllocBufferHeader*) kPointer_ByteOffset(block, (kSSize)(xkPoolAllocBlockHeader_Space() + rankInfo->bufferStride*block->allocCount)); 
...
kFx(kStatus) xkPoolAlloc_ReturnBlockToUnassigned(kPoolAlloc object, xkPoolAllocBlockList* list, xkPoolAllocBlockHeader* block)
{
    ...
    {
        xkPoolAllocBufferHeader* buffer = (xkPoolAllocBufferHeader*) kPointer_ByteOffset(block, (kSSize)(xkPoolAllocBlockHeader_Space() + i*rankInfo->bufferStride)); 
...
kFx(kStatus) xkPoolAlloc_AllocCachedBuffers(kPoolAlloc object, kSize rank, kSize count)
{
    ...
    xkPoolAllocRank* rankInfo = &obj->rankInfo[rank]; 
    kAlloc innerAlloc = kObject_Alloc(object); 
    kSize allocSize = xkPoolAllocBufferHeader_Space() + rankInfo->size; 
    kSize i; 
...
kFx(kStatus) xkPoolAlloc_AllocUnmanagedBuffer(kPoolAlloc object, kSize size, void* mem, kMemoryAlignment alignment)
{
    kObj(kPoolAlloc, object); 
    kAlloc innerAlloc = kObject_Alloc(object); 
    kSize allocSize = xkPoolAllocBufferHeader_Space() + size; 
    xkPoolAllocBufferHeader* buffer = kNULL; 
