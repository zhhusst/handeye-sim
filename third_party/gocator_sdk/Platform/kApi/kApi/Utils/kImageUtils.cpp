...
static kStatus xkImageUtils_FillDataToImage(LodePNGColorType colortype, kType type, const kImage image, const unsigned char* pixelReader)
{
    ...
    if (colortype == LCT_PALETTE && kTypeOf(k8u))
    {
        return xkImageUtils_FillData8uToImage(image, pixelReader);
    }
    ...
    *dataPng = kNULL;
    ...
    if (lodepngError == 0)
    {
        ...
        else
        {
            ...
            if (lodepngError==0 && state->info_png.color.colortype == LCT_PALETTE && state->info_png.color.bitdepth == 8)
            {
                // convert to k8u if image is gray scale image
                auto pixelReader = *dataPng;

                kBool grayScale = kTRUE;
                for (kSize i = 0; i < *height * *width; i++)
                {
                    if (pixelReader[0] != pixelReader[1] ||
                        pixelReader[0] != pixelReader[2])
                    {
                        grayScale = kFALSE;
                        break;
                    }
                ...
                if (grayScale)
                {
                    ...
                    lodepngError = lodepng_decode_memory(dataPng, width, height, png, pngSize, LCT_GREY, state->info_png.color.bitdepth);                   
                    if(lodepngError == 0)
...
static void xkImageUtils_FillDataRgb24FromImage(const kImage image, k8u* pixelWriter)
{
    const auto width = kImage_Width(image);
    const auto height = kImage_Height(image);
    ...
    for (kSize y = 0; y < height; y++)
    {
        auto rowReader = kImage_RowAtT(image, y, kRgb24);
        const auto rowReaderEnd = rowReader + width;
...
static kStatus xkImageUtils_FillDataFromImage(kImage image, kArray1 imageArray)
{
    ...
    {
        xkImageUtils_FillDataRgb24FromImage(image, data);
    }
    else if (type == kTypeOf(kArgb))
    {
...
kFx(kStatus) xkImageUtils_SaveBmp(kImage image, const kChar* fileName, kAlloc allocator)
{
    ...
    else if (type == kTypeOf(kRgb24))   outPixelSize = 3;
...
kFx(kStatus) xkImageUtils_LoadBmp(kImage* image, const kChar* fileName, kAlloc allocator)
{
    ...
    kTry
    {
        ...
        if (infoHeaderSize == 40)
        {
            ...
            kTest(kSerializer_Read32s(serializer, &height));
            if (height < 0)// If height is negative, the bitmap is a top-down 
            {
                height *= -1;
                topDown = true;
            }
...
// PGM (Portable Gray Map) functions

static kStatus xkImageUtils_MaxGrayValue(kImage image, k16u& maxValue)
{
    auto* const type = kImage_PixelType(image);
    const auto width = kImage_Width(image);
    const auto height = kImage_Height(image);
    ...
    if (type == kTypeOf(k8u))
    {
        for (kSize y = 0; y < height; y++)
        {
            auto* rowReader = kImage_RowAtT(image, y, k8u);
            auto* const rowReaderEnd = rowReader + width;

            for (; rowReader != rowReaderEnd; rowReader++)
            {
                maxValue = kMax(*rowReader, (k8u)maxValue);
            }
        }
    }
    else if (type == kTypeOf(k16u))
    {
        for (kSize y = 0; y < height; y++)
        {
            ...
            for (; rowReader != rowReaderEnd; rowReader++)
            {
                maxValue = kMax(*rowReader, maxValue);
            }
        }
    }
    else
        return kERROR; // Only gray scale is supported

    return kOK;
}

kFx(kStatus) xkImageUtils_SavePgm(kImage image, const kChar* fileName, kAlloc allocator)
{
    auto* const type = kImage_PixelType(image);
    const auto width = kImage_Width(image);
    const auto height = kImage_Height(image);
    ...
    kText256 buffer;
    k16u maxValue = 0;
    kArray1 rowBuffer = kNULL;

    kTry
    {
        kTest(xkImageUtils_MaxGrayValue(image, maxValue));
        kTest(kStrPrintf(buffer, kCountOf(buffer), "%d ", maxValue));

        kTest(kArray1_Construct(&rowBuffer, kTypeOf(kChar), width * kStrLength(buffer) + 1, allocator));

        kTest(kFile_Construct(&file, fileName, kFILE_MODE_WRITE, allocator));

        kTest(kStrPrintf(buffer, kCountOf(buffer), "P2\n"));
        kTest(kStream_Write(file, buffer, kStrLength(buffer)));

        kTest(kStrPrintf(buffer, kCountOf(buffer), "%d %d\n%d\n", width, height, maxValue));
        kTest(kStream_Write(file, buffer, kStrLength(buffer)));
        ...
        if (type == kTypeOf(k8u))
        {
            for (kSize y = 0; y < height; y++)
            {
                ...
                auto* str = kArray1_DataT(rowBuffer, kChar);
                for (; rowReader != rowReaderEnd; rowReader++)
                {
                    kTest(kStrPrintf(str + position, kArray1_DataSize(rowBuffer) - position, "%d ", *rowReader));
                    const auto strLength = kStrLength(str + position);
                    position += strLength;
                }
                if (position > 0)
                {
                    str[position-1] = '\n';
                    kTest(kStream_Write(file, kArray1_DataT(rowBuffer, kChar), position));
                }
            }
        }
        else if (type == kTypeOf(k16u))
    ...
    kFinally
    {
        ...
        kCheck(kObject_Destroy(rowBuffer));
...
kFx(kStatus) xkImageUtils_Save(kImage image, const kChar* fileName, kAlloc allocator)
{
    ...
    if (kStrCompareLower(extension, "pgm") == 0)
    {
        return xkImageUtils_SavePgm(image, fileName, alloc);
    }
