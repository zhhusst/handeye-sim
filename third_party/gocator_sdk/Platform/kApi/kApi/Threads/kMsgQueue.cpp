...
kFx(kStatus) kMsgQueue_AddEx(kMsgQueue queue, void* item, kMsgQueueItemOption options)
{
    ...
    kTry
    {
        ...
        if ((options & kMSG_QUEUE_ITEM_OPTION_INBAND) == 0)
        {
            obj->size += itemSize; 
        }
        else
        ...
        if (((obj->count - obj->inbandCount) > obj->maxCount) || (obj->size > obj->maxSize))
        {
    ...
    while (shouldContinue)
    {
        ...
        kLock_Enter(obj->lock); 
        {
            ...
            else
            {
                ...
                if ((entryObj->options & kMSG_QUEUE_ITEM_OPTION_INBAND) == 0)
                {
                    obj->size -= itemSize; 
                }
                else
...
kFx(kStatus) kMsgQueue_PurgeEx(kMsgQueue queue, kMsgQueuePurgeOption options)
{
    ...
    kTry
    {    
        kTest(xkMsgQueue_Prune(queue, options | xkMSG_QUEUE_PURGE_OPTION_COMPLETE, 0, 0)); 
    }
    kFinally
    {
        kLock_Exit(obj->lock);        
...
{
    kObj(kMsgQueue, queue); 
    kBool shouldComplete = ((options & xkMSG_QUEUE_PURGE_OPTION_COMPLETE) != 0);
    ...
    kSize i = 0; 
    kSize dropCount = 0; 
    kSize inbandDropCount = 0;
    kMsgQueueDropArgs args; 

    while (((((obj->count - obj->inbandCount) > maxCount) || (obj->size > maxSize)) || shouldComplete) && (it != itEnd))
    {
    ...
    {
        obj->dropCount += (dropCount - inbandDropCount); 
    }
...
kFx(kSize) kMsgQueue_Count(kMsgQueue queue)
{
    ...
    kLock_Enter(obj->lock); 
    {
        count = obj->count - obj->inbandCount; 
    }
    kLock_Exit(obj->lock); 
