...
kBeginClassEx(k, kWebSocket)
    ...
    kAddPrivateVMethod(kWebSocket, kStream, VFlush)
    kAddPrivateVMethod(kWebSocket, kStream, VFill)
    kAddPrivateVMethod(kWebSocket, kStream, VSeek)
kEndClassEx()
...
kFx(kStatus) kWebSocket_EnableSeek(kWebSocket webSocket, kBool enabled)
{
    kObj(kWebSocket, webSocket);
...
kFx(kStatus) xkWebSocket_VSeek(kWebSocket webSocket, k64s offset, kSeekOrigin origin)
{
    kObj(kWebSocket, webSocket);
    ...
    case kSEEK_ORIGIN_BEGIN:
        offset -= (k64s)kStream_BytesRead(webSocket); 
        break;
    ...
    if (offset >= 0)
    {
        ...
        while (offset > 0)
        {
            ...
            kCheck(kStream_Read(webSocket, buffer, (kSize)chunkSize));
...
kFx(kStatus) xkWebSocket_ReadAtLeast(kWebSocket webSocket, kByte* buffer, kSize minCount, kSize maxCount, kSize* bytesRead)
{
    ...
    kTry
    {
        while (totalBytesRead < minCount)
        {
            ...
            minRequest = kMin_(minCount - totalBytesRead, payloadRemaining);
            maxRequest = kMin_(maxCount - totalBytesRead, payloadRemaining);
...
kFx(kTcpClient) kWebSocket_TcpClient(kWebSocket webSocket)
{
    kObj(kWebSocket, webSocket);

    return obj->tcpClient;
}
