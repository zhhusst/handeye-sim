...
kFx(kStatus) kStream_CopyImpl(kStream stream, kStream source, kSize size, kCallbackFx progress, kPointer context, kSize bufferSize)
{
    kArray2 buffer = kNULL;
    kSize bytesCopied = 0; 
    k64u updateTime = k64U_NULL; 

    kCheckArgs(bufferSize != 0);
...
{
    return kStream_CopyImpl(stream, source, size, progress, context);
}
