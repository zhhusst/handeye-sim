 ...
 * 
 * Typically, the serializer will flush after its first buffer is filled. However, if the 
 * BeginWrite/EndWrite methods are used to emit measured sections, additional buffers of the 
 * same size may be allocated to maintain content until the measured write is complete.
 *
 * @public                  @memberof kSerializer
 * @param   serializer      Serializer object. 
 * @param   size            Individual buffer size, in bytes.
 * @return                  Operation status. 
 */
kFx(kStatus) kSerializer_SetBufferSize(kSerializer serializer, kSize size);
 ...
 * @public                  @memberof kSerializer
 * @param   serializer      Serializer object. 
 * @return                  Individual buffer size, in bytes.
 */
kInlineFx(kSize) kSerializer_BufferSize(kSerializer serializer)
{
 ...
 * 
 * Writes larger than the specified size will trigger a flush to the underlying stream, followed by a 
 * direct write of the provided buffer (provided that the serializer is currently in a state in which 
 * it can be flushed).
 *
 * @public                  @memberof kSerializer
 * @param   serializer      Serializer object. 
 * @param   size            Maximum buffered write size, in bytes.
 * @return                  Operation status. 
 */
kInlineFx(kStatus) kSerializer_SetMaxBufferedWriteSize(kSerializer serializer, kSize size)
{
    kObj(kSerializer, serializer);

    obj->maxBufferedWriteSize = size; 
 ...
 * @public                  @memberof kSerializer
 * @param   serializer      Serializer object. 
 * @return                  Maximum buffered write size, in bytes. 
 */
kInlineFx(kSize) kSerializer_MaxBufferedWriteSize(kSerializer serializer)
{
    kObj(kSerializer, serializer);

    return obj->maxBufferedWriteSize;
}
 ...
 * @public                  @memberof kSerializer
 * @param   serializer      Serializer object. 
 * @param   enable          kTRUE to enable automatic flush after each object.
 * @return                  Operation status. 
 */
kInlineFx(kStatus) kSerializer_EnableObjectFlush(kDat6Serializer serializer, kBool enable)
{
    kObj(kSerializer, serializer);

    obj->objectFlushEnabled = enable;
 ...
 * @public                  @memberof kSerializer
 * @param   serializer      Serializer object. 
 * @return                  kTRUE if automatic flush is enabled.
 */
kInlineFx(kBool) kSerializer_ObjectFlushEnabled(kDat6Serializer serializer)
{
    kObj(kSerializer, serializer);

    return obj->objectFlushEnabled;
}
 ...
 * @public              @memberof kSerializer
 * @param   serializer  Serializer object. 
 * @param   offset      Number of bytes to write. 
 * @return              Operation status. 
 */
kInlineFx(kStatus) kSerializer_AdvanceWrite(kSerializer serializer, kSize offset)
{
    if (offset == 0)
    {
        return kOK; 
    ...
    return xkSerializer_AdvanceWriteImpl(serializer, offset); 
}
