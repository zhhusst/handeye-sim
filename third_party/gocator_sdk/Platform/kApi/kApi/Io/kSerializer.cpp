...
kFx(kStatus) kSerializer_VInit(kSerializer serializer, kType type, kStream stream, kAlloc allocator)
{
    ...
    obj->readAlloc = kNULL;
    obj->objectFlushEnabled = kTRUE; 
    obj->endianness = kENDIANNESS_LITTLE; 
    obj->swap = kEndianness_ShouldReverse(obj->endianness); 
    obj->sizeEncoding = 4; 
    obj->maxBufferedWriteSize = xkSERIALIZER_DEFAULT_MAX_BUFFERED_WRITE_SIZE;
    obj->format[0] = 0;
...
kFx(kStatus) kSerializer_VRelease(kSerializer serializer)
{
    ...
    kCheck(xkSerializer_FreeBuffers(serializer));
...
kFx(kStatus) xkSerializer_AdvanceWriteImpl(kSerializer serializer, kSize offset)
{
    kObj(kSerializer, serializer);
    kByte bitBucket[128]; 
    kSize remaining = offset; 
    ...
    while (remaining > 0)
    {
        kSize writeAmount = kMin_(remaining, sizeof(bitBucket)); 

        kCheck(kStream_Write(obj->writeStream, bitBucket, writeAmount));
...
kFx(kStatus) xkSerializer_FreeBuffers(kSerializer serializer)
{
    kObj(kSerializer, serializer);
 
    kCheck(xkSerializer_ClearBuffers(serializer)); 
 
    xkSerializerBuffer* buffer = obj->freeBuffers; 
...
kFx(kStatus) xkSerializer_WritePrimitives(kSerializer serializer, const void* items, kSize count, xkSerializerPrimative primitive)
{
    ...
    if (autoFlush && !obj->swap && (count > obj->maxBufferedWriteSize))
    {
        kCheck(xkSerializer_FlushBuffers(serializer));
