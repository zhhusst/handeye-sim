 ...
 * kIpAssignment enum
 */
kBeginEnumEx(k, kIpAssignment)
    kAddEnumerator(kIpAssignment, kIP_ASSIGNMENT_UNKNOWN)
    kAddEnumerator(kIpAssignment, kIP_ASSIGNMENT_AUTOMATIC)
    kAddEnumerator(kIpAssignment, kIP_ASSIGNMENT_MANUAL)
kEndEnumEx()
...
kFx(kStatus) xkNetworkAdapter_Construct(kNetworkAdapter* adapter, kNetworkInfo info, const kChar* name, kSize id, kBool isUp, kMacAddress macAddress, kIpAssignment ipAssignment, kAlloc allocator)
{
    kAlloc alloc = kAlloc_Fallback(allocator);
    kStatus status; 
    ...
    if (!kSuccess(status = xkNetworkAdapter_Init(*adapter, kTypeOf(kNetworkAdapter), info, name, id, isUp, macAddress, ipAssignment, alloc)))
    {
...
kFx(kStatus) xkNetworkAdapter_Init(kNetworkAdapter adapter, kType type, kNetworkInfo info, const kChar* name, kSize id, kBool isUp, kMacAddress macAddress, kIpAssignment ipAssignment, kAlloc alloc)
{
...
kFx(kStatus) xkNetworkInfo_AddAdapter(kNetworkInfo info, const kChar* name, kSize id, kBool isUp, kMacAddress macAddress, kIpAssignment ipAssignment, kNetworkAdapter* adapter)
{    
    kObj(kNetworkInfo, info); 
    ...
    kTry
    {
        kTest(xkNetworkAdapter_Construct(&newAdapter, adapter, name, id, isUp, macAddress, ipAssignment, kObject_Alloc(info)));
...
kFx(kStatus) kNetwork_FindMacAdressByInterface(kIpAddress address, kMacAddress* macAddress)
{
    kNetworkInfo info = kNULL;
    kNetworkInterface iface = kNULL;
    ...
    kTry
    {
        ...
        if (!kIsNull(macAddress))
        {
            *macAddress = kNetworkAdapter_MacAddress(kNetworkInterface_Adapter(iface));
        }
    }
    kFinally
    {
...
kFx(kStatus) xkNetwork_DefaultQueryNetInfo(kNetworkInfo info)
{
    ...
    kTry
    {
        ...
        while (!kIsNull(it))
        {
            ...
            kIpAssignment ipAssignment = it->Dhcpv4Enabled ? kIP_ASSIGNMENT_AUTOMATIC : kIP_ASSIGNMENT_MANUAL;
#else
            kIpAssignment ipAssignment = kIP_ASSIGNMENT_MANUAL;
#endif
            ...
            kTest(xkNetworkInfo_AddAdapter(info, name, it->IfIndex, isUp, macAddress, ipAssignment, &adapter));
...
static kStatus xkNetwork_Sanitize(kNetworkInfo info)
{
    for (kSize i = 0; i < kNetworkInfo_AdapterCount(info); ++i)
    {
        kNetworkAdapter adapter = kNetworkInfo_AdapterAt(info, i);

        //FPT-2085: kNetworkInfo for Linux is not consistent with VxWorks
        //When the adapter IP address is 0.0.0.0; the caller will have created 
        //an adapter without interfaces. Let's sanitize the adapter to be 
        //consistent with VxWorks.

        if (kNetworkAdapter_InterfaceCount(adapter) == 0)
        {
            kCheck(xkNetworkAdapter_AddInterface(adapter, kIpAddress_AnyV4(), 0, kNULL));
        }
    }
        ...
        while (!kIsNull(ifaListIt))
        {
            ...
            if ((ifaListIt->ifa_addr != kNULL))
            {
                if (!kSuccess(kNetworkInfo_FindAdapterByName(info, name, &adapter)))
                {
                    ...
                    if (ifaceReq.ifr_hwaddr.sa_family == ARPHRD_ETHER ||
                        ifaceReq.ifr_hwaddr.sa_family == ARPHRD_LOOPBACK)
                    {
                        kTest(xkNetworkInfo_AddAdapter(info, name, id, isUp, macAddress, kIP_ASSIGNMENT_UNKNOWN, &adapter));
                    }
                }

                if (ifaListIt->ifa_addr->sa_family == AF_INET)
                {
...
kFx(kBool) xkNetwork_DefaultQueryNetChange(k64u timeout)
{
    ...
    {        
        if (kSuccess(kSocket_ReadFrom(sobj->netlinkSocket, kNULL, sobj->buffer, sizeof(sobj->buffer), &sobj->bytesRead)))
        {
            return kTRUE;
        }
    }
