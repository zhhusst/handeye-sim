 ...
 *
 * @internal
 * Copyright (C) 2004-2024 by LMI Technologies Inc.
...
kFx(kStatus) xkSocket_Init(kSocket sockt, kType type, kIpVersion ipVersion, kSocketType socketType, kAlloc allocator)
{
    ...
    else
    {
        ...
        xkSocketHandle handle = WSASocket(sockFamily, sockType, 0, NULL, 0, dwFlags);
#elif defined (K_LINUX)
        xkSocketHandle handle = socket(sockFamily, sockType | SOCK_CLOEXEC, 0);
#else
