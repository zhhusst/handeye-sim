...
kInlineFx(kStatus) xkMap_AddItemT(kMap map, const void* key, const void* value, kMapItem* item, kSize keySize, kSize valueSize)
{
    kAssert(xkType_IsPointerCompatible(kMap_KeyType(map), keySize)); 
    kAssert(xkType_IsPointerCompatible(kMap_ValueType(map), valueSize)); 

    return kMap_AddItem(map, key, value, item);
} 
