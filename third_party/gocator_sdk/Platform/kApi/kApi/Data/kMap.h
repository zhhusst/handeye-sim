 ...
 */
kInlineFx(kStatus) kMap_Add(kMap map, const void* key, const void* value)
{
    return kMap_AddItem(map, key, value, kNULL); 
}
 ...
 * @return              kOK if added; kERROR_ALREADY_EXISTS if key already present. 
 */
kFx(kStatus) kMap_AddItem(kMap map, const void* key, const void* value, kMapItem* item); 
 ...
 * @return                      kOK if added; kERROR_ALREADY_EXISTS if key already present. 
 */
#define kMap_AddItemT(kMap_map, KPtr_key, VPtr_value, kMapItemPtr_item) \
    xkMap_AddItemT(kMap_map, KPtr_key, VPtr_value, kMapItemPtr_item, sizeof(*(KPtr_key)), sizeof(*(VPtr_value)))
