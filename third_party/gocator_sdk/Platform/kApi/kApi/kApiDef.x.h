...
kForwardDeclareClass(k, kDataTree, kObject)
kForwardDeclareClass(k, kDataTreeItem, kObject)
...
kForwardDeclareClass(k, kHeapAlloc, kAlloc)
...
kDeclareValueEx(k, kRgb24, kValue)

kFx(kBool) xkRgb24_VEquals(kType type, const void* value, const void* other);
kFx(kSize) xkRgb24_VHashCode(kType type, const void* value);
kFx(kStatus) xkRgb24_Write(kType type, const void* values, kSize count, kSerializer serializer);
kFx(kStatus) xkRgb24_Read(kType type, void* values, kSize count, kSerializer serializer);

#define xkRgb24_Init_(RGB24, R, G, B)                          \
    ((RGB24)->b = (B), (RGB24)->g = (G), (RGB24)->r = (R))
...
//[Deprecated] Simply use kExport. The word "Class" is not great, because in C++ we also have structs and enums.
#define kExportClass kExport

//[Deprecated] Simply use kImport. The word "Class" is not great, because in C++ we also have structs and enums.
#define kImportClass kImport
