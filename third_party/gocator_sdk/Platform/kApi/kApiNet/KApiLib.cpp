...
#include "kApiNet/Data/KDataTree.h"
...
KDefineClass(KDataTree, kDataTree)
KDefineClass(KDataTreeItem, kDataTreeItem)
...
kStatus kCall Lmi3d::Zen::KApiLib_OnAlloc(kPointer provider, kSize size, void* mem, kMemoryAlignment alignment)
{
    ...
    alignment = Math::Max(alignment, kALIGN_ANY); 

    try
    {
        kSize headerSize = kSize(1) << alignment;
        kSize totalAllocationSize = headerSize + size;

        kAssert(headerSize >= 2*sizeof(kSize));
