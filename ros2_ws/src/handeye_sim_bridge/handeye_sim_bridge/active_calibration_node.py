#!/usr/bin/env python3
"""Backend-neutral initialization, MoveIt planning, NBV execution and update.

The node consumes a normalized measurement contract. Simulation truth is an
optional evaluator and is never required by the solver in real-hardware mode.
"""
...
DEFAULT_HAND_EYE_ROTATION = [
    0.060489873205236855,
    0.8875159337280827,
    -0.4567892759450289,
    -0.9852687179744033,
    -0.020246266544824032,
    -0.16981060647695614,
    -0.15995789639992802,
    0.46033200634951,
    0.8732169921099476,
]
DEFAULT_HAND_EYE_TRANSLATION = [-0.011579, -0.004621, 0.359284]
...
def effective_measurement_timeout_s(
    ...
    """Give a requested frame batch enough time under a loaded backend."""
...
class ActiveCalibrationNode(Node):
    ...
    def __init__(self) -> None:
        ...
        self.declare_parameter("runtime.backend", "simulation")
        self.declare_parameter(
            "initial_handeye.rotation", DEFAULT_HAND_EYE_ROTATION
        )
        self.declare_parameter(
            "initial_handeye.translation_m", DEFAULT_HAND_EYE_TRANSLATION
        )
        self.declare_parameter("evaluation.truth_available", True)
        self.declare_parameter(
            "evaluation.handeye_rotation", DEFAULT_HAND_EYE_ROTATION
        )
        self.declare_parameter(
            "evaluation.handeye_translation_m", DEFAULT_HAND_EYE_TRANSLATION
        )
        self.declare_parameter(
            "interfaces.noise_status_service", "/scene_publisher/noise_status"
        )
        ...
        self.declare_parameter(
            "interfaces.plan_service", "/plan_kinematic_path"
        )
        self.declare_parameter(
            "interfaces.validity_service", "/check_state_validity"
        )
        self.declare_parameter(
        ...
        self.backend = str(
            self.get_parameter("runtime.backend").value
        ).strip().lower()
        nominal_rotation_values = np.asarray(
            self.get_parameter("initial_handeye.rotation").value,
            dtype=float,
        )
        nominal_translation_values = np.asarray(
            self.get_parameter("initial_handeye.translation_m").value,
            dtype=float,
        )
        if nominal_rotation_values.size != 9:
            raise ValueError("initial_handeye.rotation must contain 9 values")
        if nominal_translation_values.size != 3:
            raise ValueError(
                "initial_handeye.translation_m must contain 3 values"
            )
        self.nominal_handeye_rotation = nominal_rotation_values.reshape(3, 3)
        self.nominal_handeye_translation = nominal_translation_values.reshape(3)
        self.truth_handeye_rotation: np.ndarray | None = None
        self.truth_handeye_translation: np.ndarray | None = None
        if bool(self.get_parameter("evaluation.truth_available").value):
            truth_rotation_values = np.asarray(
                self.get_parameter("evaluation.handeye_rotation").value,
                dtype=float,
            )
            truth_translation_values = np.asarray(
                self.get_parameter("evaluation.handeye_translation_m").value,
                dtype=float,
            )
            if truth_rotation_values.size != 9:
                raise ValueError(
                    "evaluation.handeye_rotation must contain 9 values"
                )
            if truth_translation_values.size != 3:
                raise ValueError(
                    "evaluation.handeye_translation_m must contain 3 values"
                )
            self.truth_handeye_rotation = truth_rotation_values.reshape(3, 3)
            self.truth_handeye_translation = truth_translation_values.reshape(3)
        ...
        # The solver consumes only the selected calibration-target surface;
        # workbench and plate-side returns remain on the raw profile topic.
        ...
        self.plan_client = self.create_client(
            GetMotionPlan,
            str(self.get_parameter("interfaces.plan_service").value),
        )
        ...
        if self.backend == "simulation":
            self.noise_status_client = self.create_client(
                ...
                str(
                    self.get_parameter(
                        "interfaces.noise_status_service"
                    ).value
                ),
            )
        ...
        self.trajectory_client = ActionClient(
            ...
            str(self.get_parameter("interfaces.trajectory_action").value),
        )
        ...
        self.get_logger().info(
            f"{self.backend} active calibrator waiting for {self.seed_file}"
        )
    ...
    def _initialize_from_seeds(self) -> None:
        try:
            ...
            nominal_rotation = self.nominal_handeye_rotation @ so3_exp(
                rotation_axis * angle
            )
            nominal_translation = (
                self.nominal_handeye_translation
                + translation_axis * translation_error
            )
            ...
            if not result.converged:
                self._fail(
                    ...
                    f"data_rank={result.diagnostics.rank}, "
                    "data_condition="
                    f"{result.diagnostics.condition_number:.3e}"
                )
                return
            ...
            truth_text = self._truth_error_text(result)
            self.get_logger().info(
                f"12-DOF-V2 initialized ({result.diagnostics.surface_model}): "
                f"data_rank={result.diagnostics.rank}, "
                f"data_condition={result.diagnostics.condition_number:.3e}, "
                "prior_rank="
                f"{result.diagnostics.prior_augmented_rank}, "
                "prior_condition="
                f"{result.diagnostics.prior_augmented_condition_number:.3e}, "
    ...
    def _request_motion_plan(self, goal_joints: np.ndarray) -> None:
        ...
        constraints.joint_constraints = [
            ...
            for name, position in zip(self.joint_names, goal_joints)
        ]
    ...
    def _publish_detection_prior(self) -> None:
        if self.executing_score is None:
            return
        measurement = self.executing_score.prediction.measurement
        ...
        endpoints = np.vstack(
            (measurement.endpoint_u, measurement.endpoint_v)
        ).astype(np.float32)
        # NBV uses a model-predicted one-shot association at the settled target
        # pose.  It must not inherit the continuous seed-stage Kalman state.
        ...
        self.get_logger().info(
            "published finite predicted NBV segment to the real-profile "
            "association gate"
        )
    ...
    def _finish(self) -> None:
        ...
        self.state = "DONE"
        self.get_logger().info(
            f"{self.backend} calibration complete; retained latest committed "
            f"estimate at NBV {self.pipeline.nbv_count} -> {self.output_file}"
        )

    def _truth_errors(self, result) -> tuple[float | None, float | None]:
        if (
            self.truth_handeye_rotation is None
            or self.truth_handeye_translation is None
        ):
            return None, None
        rotation_error = rotation_distance_deg(
            result.estimate.handeye_rotation,
            self.truth_handeye_rotation,
        )
        translation_error = 1000.0 * float(
            np.linalg.norm(
                result.estimate.handeye_translation
                - self.truth_handeye_translation
            )
        )
        return float(rotation_error), float(translation_error)

    def _truth_error_text(self, result) -> str:
        rotation_error, translation_error = self._truth_errors(result)
        if rotation_error is None or translation_error is None:
            return "truth_error=unavailable"
        return (
            f"rotation_error={rotation_error:.4f} deg, "
            f"translation_error={translation_error:.4f} mm"
        )
    ...
    def _record_iteration(self, phase: str, result, score=None) -> None:
        ...
        record = {
            ...
            "data_only_rank": int(result.diagnostics.rank),
            "data_only_condition_number": float(
                result.diagnostics.condition_number
            ),
            "prior_augmented_rank": int(
                result.diagnostics.prior_augmented_rank
                if result.diagnostics.prior_augmented_rank is not None
                else result.diagnostics.rank
            ),
            "prior_augmented_condition_number": float(
                result.diagnostics.prior_augmented_condition_number
                if result.diagnostics.prior_augmented_condition_number
                is not None
                else result.diagnostics.condition_number
            ),
    ...
    def _poll_noise_status(self) -> None:
        ...
        if self.backend != "simulation" or self.noise_status_client is None:
            return
