...
def covariance_from_jacobian(
    ...
    *,
    ...
    variance_residual: np.ndarray | None = None,
    ...
    variance_values = (
        ...
        if variance_residual is None
        else np.asarray(variance_residual, dtype=float)
    )
    ...
    # be included in the residual-variance degrees of freedom. A regularizing
    # prior may contribute to the posterior Hessian, but must not be counted
    # as another noisy measurement when estimating measurement variance.
    dof = max(
        len(variance_values)
        - jacobian.shape[1]
    ...
    variance = max(
        float(variance_values @ variance_values / dof), variance_floor
    )
